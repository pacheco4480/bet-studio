...
export function renderHtmlForTesting(
  model: BulletinRenderModel,
  template: RenderTemplateVersion,
): string {
...
function renderHtml(
  ...
  const cards = model.selections
    .map((selection) => {
      ...
      const marketMaxWidth =
        mode.id === 'ULTRA_COMPACT' ? 650 : mode.id === 'LARGE' ? 690 : 560;
      ...
      return `<section class="selection-card ${mode.id.toLowerCase()}">
        ...
        <div class="result-row">
          ...
          ${model.display.showResult ? `<span class="status ${selection.status.toLowerCase()}">${selection.status}</span>` : ''}
        </div>
      </section>`;
    })
    .join('');
