...
function renderHtml(
  ...
  const cards = model.selections
    .map((selection) => {
      ...
      return `<section class="selection-card ${mode.id.toLowerCase()}">
        ...
        <div class="teams-row">
          <div class="team">
            <div class="logo">${initials(selection.homeTeam.name)}</div>
          ...
          <div class="team away">
            ...
            <div class="logo">${initials(selection.awayTeam.name)}</div>
          </div>
        </div>
  ...
  <html lang="pt">
    <head>
      <meta charset="utf-8" />
      <style>${renderCss(template, mode.cardHeight, mode.cardGap)}</style>
    </head>
    <body>
